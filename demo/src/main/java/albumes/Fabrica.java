...
public class Fabrica {
	...
	Album crearAlbumWeb(String codigoPromocional, int codigoUnico, String nombre, int dni) {
		if (codigoPromocional.length() < 6 || codigoPromocional.length() > 8 || codigoUnico == 0 || nombre.length() > 30
				|| dni < 0 || 99999999 < dni) {
			throw new RuntimeException("Error: se esta intentando construir un albumWeb con parametros incorrectos.\n");
		}
	...
	Album crearAlbumExtendido(int codigoUnico, String nombre, int dni) {
		if (codigoUnico == 0 || nombre.length() > 30 || dni < 0 || dni > 99999999) {
			throw new RuntimeException(
					"Error: se esta intentando construir un albumExtendido con parametros incorrectos.\n");
		}
	...
	Album crearAlbumTradicional(int codigoUnico, String nombre, int dni) {
		if (codigoUnico == 0 || nombre.length() > 30 || dni < 0 || dni > 99999999) {
			throw new RuntimeException(
					"Error: se esta intentando construir un albumTradicional con parametros incorrectos.\n");
		}
	...
	ArrayList<Figurita> generarSobre(int cantFigus) {
		...
		int hasta = cantFigus;
		while (hasta > 0) {
			int rand = (int) ((cantPaises * lugaresPorPais) * Math.random());// habia un error logico aca, habia puesto
																				// 383, y no completaba el album
	...
	ArrayList<FiguritaTop10> generarSobreTop10(int cantFigus) {
		...
		if (this.coleccion20[9] == null) {
			System.out.println("aca no hay nada");
		}
		int hasta = cantFigus;
		while (hasta > 0) {
			int rand = (int) (20 * Math.random());// habia un error logico aca, habia puesto 383, y no completaba el
													// album
	...
	private FiguritaTop10[] generarColeccion20() {
		...
		for (int i = 19; i >= 0; i--) {
			...
			coleccion200[i] = new FiguritaTop10(this.nombres[i] + " " + this.apellidos[i], "Extendido",
					balonYPaisPorMundialTop10.get(listadoDeMundialesTop10[i / 2])[i % 2],
					ranking.get(balonYPaisPorMundialTop10.get(listadoDeMundialesTop10[i / 2])[i % 2]), i + 384,
					listadoDeMundialesTop10[i / 2], balond);
		}
		...
		for (int i = 0; i < this.nombres.length; i++) {
			...
			for (int j = 0; j < this.apellidos.length; j++) {
				...
				if (contandoPaises < cantPaises * lugaresPorPais) {
					// Figurita nueva=
					coleccioncompleta[contandoPaises] = new Figurita(nombre, null,
							paisesParticipantes[contandoPaises / 12],
							ranking.get(paisesParticipantes[contandoPaises / 12]), contandoPaises);
