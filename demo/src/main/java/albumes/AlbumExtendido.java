...
public ArrayList<String>  verificarYpegarFigus(){//bueno aca use Iterator asi que no se pueden quejar
      ...
      if (this.AlbumCompleto==true){return null;}
          ...
          if(actual.getNumeroQueIdentifica()<coleccion.length && coleccion[actual.getNumeroQueIdentifica() ]==null&& actual.getTipoDeFigu().equals(this.getTipoAlbum())){//si la posicion es null, quiere decir que la figu no esta pegada.
              ...
              else if (actual.getNumeroQueIdentifica()<coleccion.length &&!coleccion[actual.getNumeroQueIdentifica()].equals(actual)){//si esta pegada pero es distinta.
              ...
              else if(!(actual.getNumeroQueIdentifica()<coleccion.length)&& coleccion20[actual.getNumeroQueIdentifica()-384 ]==null&& actual.getTipoDeFigu().equals(this.getTipoAlbum())){

                coleccion20[actual.getNumeroQueIdentifica()-384]=(FiguritaTop10)actual;//agrego al array, espero no me de problema de ejecucion.
                ret.add(actual.toString());//Agrego la figu a la lista de retorno
                algo.remove();
...
public void mostrarPegadas(){
        ...
        for (int i=0;i<coleccion20.length;i++){ if(coleccion20[i]!=null){
